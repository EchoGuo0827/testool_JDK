...
public class SysTest88 extends DefaultFragment {
	
	private final String TESTITEM = "(ScanUtil)startYUVDecode方式扫码综合";
	private final String TAG = SysTest88.class.getSimpleName();
...
//					isTheTimeEnd=true;
		...
		while(true)
		{
			int nkeyIn = gui.cls_show_msg("startYUV方式扫码压力综合\n0.扫码配置\n1.扫码压力\n2.扫码性能\n3.扫码异常测试");
			switch (nkeyIn) 
			{
			case '0':
				cameIdConfig();
			...
			case '1':
				...
				int nkeyIn2 = gui.cls_show_msg("startYUV方式扫码压力\n0.无释放Camera扫码压力\n1.释放Camera扫码压力\n2.休眠唤醒扫码压力\n");
				switch (nkeyIn2) {
				case '0':
					noReleaseScanPre(mResultCallBack);
					break;
				case'1':
					releaseScanPre(mResultCallBack);
					break;
					
				case '2':
					sleepWakePre(mResultCallBack);
					break;
			...
			case '2':
				...
				int nkeyIn3 = gui.cls_show_msg("startYUV方式扫码性能\n0.反复扫码性能\n1.释放Camera解析码性能\n");
				switch (nkeyIn3) 
				{
				case '0':
					noReleaseScanAbility(mResultCallBack);
					break;
					
				case'1':
					releaseCameraAbility(mResultCallBack);
					break;
	...
	public void cameIdConfig()
	{
		...
		if(scanSet==2||scanSet==1)
		{
			...
			mScanUtil.setNlsScn("QR", "CodeNum", scanSet+"");
			mScanUtil.setNlsScn("QR", "NumFixed", "0");
		}
	}

	// 释放Camera的扫码压力
	private void releaseScanPre(ResultCallBack mResultCallBack) 
	{
		...
		while (cnt > 0) {
			...
			mScanUtil.initDecode(mResultCallBack);
...
//			}
	...
	// 无释放Camera的扫码压力
	private void noReleaseScanPre(ResultCallBack mResultCallBack) 
	{
	...
	//反复扫码性能
	private void noReleaseScanAbility(ResultCallBack mResultCallBack) 
	{
		...
		handler.sendEmptyMessage(HandlerMsg.SURFACEVIEW_GONE);
		timeAverage= timeTotal/succ;// 算的是成功次数
	 ...
	 * 释放Camera方式解析码的性能
	 */
	private void releaseCameraAbility(ResultCallBack mResultCallBack) 
	{
		/**测试前置，释放扫码资源*/
		releaseAll();
	 ...
	 * 休眠唤醒压力
	 */
	private void sleepWakePre(ResultCallBack mResultCallBack) 
	{
...
//				gui.cls_show_msg("【Poynt】请手动将休眠时间设置为永不休眠,设置完毕任意键继续");
	...
	public void scanAbnormal(ResultCallBack mResultCallBack)
	{
		longTimeNoCodeAbnormal(mResultCallBack);
	}
	
	// 长时间未放置条码异常测试
	public void longTimeNoCodeAbnormal(ResultCallBack mResultCallBack)
	{
		// 测试前置会删除全部的文件
		deleteSomeFile(true);
...
//			LoggerUtil.e("initCamera,width="+previewWidth+",height="+previewHeight);
//		}
//		
//		
//		setCameraDisplayOrientation(camTip,scanDefineInfo, parameters);
//		// 1、设置预览分辨率
...
//		mCamera.startPreview();
	...
	/**设置摄像头画面的预览角度*/
	private void setCameraDisplayOrientation(String campTip,ScanDefineInfo scanDefineInfo, Parameters params) {
