...
/************************************************************************
* module			: 异步modem模块
* file name			: Asyn8.java
* Author 			: zhengxq
* version			: 
* DATE				: 20141212
* directory 		: 
* description		: 测试异步modem的getreadlen能够获取modem缓冲区数据长度
* related document	: 
* history 		 	: author			date			remarks
*			  		 zhengxq		   20141212 		created
************************************************************************/
public class Asyn8 extends UnitFragment
{
	/*------------global variables definition-----------------------*/
	private final String CLASS_NAME = Asyn8.class.getSimpleName();
	private final int MAXWAITTIME = 60;
	private final int PCKMAXLEN = 350;
	private LinkType type = LinkType.ASYN;
	private Gui gui = new Gui(myactivity, handler);
	private String TESTITEM = "异步modem的getreadlen";
	private NlModemManager nlModemManager = null;
	
	public void asyn8() 
	{
		String funcName ="asyn8";
		try 
		{
		...
		{
			gui.cls_show_msg1_record(CLASS_NAME,funcName,gKeepTimeErr,"line %d:未初始化返回值错误(ret = %d)", Tools.getLineInfo(),ret);
			if(!GlobalVariable.isContinue)
				return;
		}
		...
		{
			gui.cls_show_msg1_record(CLASS_NAME,funcName,gKeepTimeErr,"line %d:检测状态出错(ret = %d)", Tools.getLineInfo(),ret);
			if(!GlobalVariable.isContinue)
				return;
		}
		...
		if(gui.cls_show_msg1(2, "刚拨号成功modem缓冲区长度应该没有数据,应该返回0,[取消]退出测试")==ESC)
			return;
		...
		if(gui.cls_show_msg1(2, "正常写数据判断modem长度是否正确,应该等于写入的数据长度,[取消]退出测试")==ESC)
			return;
		...
		if(gui.cls_show_msg1(2, "读取数据后缓冲区应该要被清空,此时读数据长度应该为0,[取消]退出测试")==ESC)
			return;
		...
		{
			gui.cls_show_msg1_record(CLASS_NAME,funcName,gKeepTimeErr,"line %d:收发长度不一致(ret = %d)", Tools.getLineInfo(),ret);
			if(!GlobalVariable.isContinue)
				return;
		}
		...
		// case5: 异步modem清缓冲区在拨号之后有效,所以应该返回0
		if(gui.cls_show_msg1(2, "异步modem清缓冲区在拨号之后有效,所以应该返回0,[取消]退出测试")==ESC)
			return;
		if ((ret = modem_write(nlModemManager, buf, PCKMAXLEN)) != NDK_OK) 
		{
			gui.cls_show_msg1_record(CLASS_NAME,funcName,gKeepTimeErr,"line %d:写数据失败(ret = %d)", Tools.getLineInfo(),ret);
		...
		{
			gui.cls_show_msg1_record(CLASS_NAME,funcName,gKeepTimeErr,"line %d:清空缓冲区失败(ret = %d)", Tools.getLineInfo(),ret);
			if(!GlobalVariable.isContinue)
				return;
		}
		SystemClock.sleep(10000);
		...
		if(gui.cls_show_msg1(2, "挂断后,缓冲区的长度应该为0,[取消]退出测试")==ESC)
			return;
		if((ret = new Layer(myactivity,handler).linkDown(nlModemManager,type))!= NDK_OK)
		{
			gui.cls_show_msg1_record(CLASS_NAME,funcName,gKeepTimeErr,"line %d:断开链路失败(ret = %d)", Tools.getLineInfo(),ret);
			if(!GlobalVariable.isContinue)
				return;
		}
		if((ret = modem_reset(nlModemManager))!=0)
		{
			gui.cls_show_msg1_record(CLASS_NAME,funcName,gKeepTimeErr,"line %d:获取modem长度错误(ret = %d)", Tools.getLineInfo(),ret);
			if(!GlobalVariable.isContinue)
				return;
