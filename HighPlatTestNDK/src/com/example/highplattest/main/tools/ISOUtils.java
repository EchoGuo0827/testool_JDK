...
//                        d.append("{NULL}");
        ...
        if (len == 4) {
            ...
            if (last >= 8) {
                throw new IllegalArgumentException("illegal input:"
                        + Dump.getHexDump(bytes, offset, len));
            }
        }
    ...
    /**
     * change simple tlv tag to follow ISO/IEC 7816 defined ISO/IEC 7816 uses
     * neither '00' nor 'FF' as tag value. Before, between, or after TLV-coded
     * data objects, '00' or 'FF' bytes without any meaning may occur (for
     * example, due to erased or modified TLV-coded data objects). by lance
     * 
     * @param tag
     * @return
     */
    public static int toBERTLVTag(int tag) {
        if (tag < 0)
            throw new IllegalArgumentException("illegal input:" + tag);

        int rslt = 0;
        int index = 0;
        for (int i = 0; i < 3; i++) {
            int temp = (tag >> (i * 8)) & 0xFF;
            if (temp != 0x00 && temp != 0xFF) {
                rslt += (temp << (index * 8));
                index++;
            }
        }
        if (rslt > 0xFFFFFF) { // max only to 3 bytes
            throw new IllegalArgumentException("only support 3 bytes tag"
                    + Dump.getHexDump(ISOUtils.intToBytes(rslt, 4, true)));
        }
        if (rslt > 0xFFFF) {
            if ((rslt & 0x1F8000) != 0x1F8000) {
                throw new IllegalArgumentException("illegal tag format:"
                        + Dump.getHexDump(ISOUtils.intToBytes(rslt, 3, true)));
            }
        }
        if (rslt > 0xFF) {
            if ((rslt & 0x1F00) != 0x1F00) {
                throw new IllegalArgumentException("illegal tag format:"
                        + Dump.getHexDump(ISOUtils.intToBytes(rslt, 2, true)));
            }
        }
        if ((rslt & 0x1F) == 0x1F) {
            throw new IllegalArgumentException("illegal tag format:"
                    + Dump.getHexDump(ISOUtils.intToBytes(rslt, 1, true)));
        }
        return rslt;
    }
