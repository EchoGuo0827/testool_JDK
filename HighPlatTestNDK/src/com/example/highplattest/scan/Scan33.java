 ...
 ************************************************************************/
public class Scan33 extends UnitFragment
{
	private final String TESTITEM =  "(SoftEngine+Camera1)setScanningCallback和stopDecode";
	private String fileName=Scan33.class.getSimpleName();
	...
	public void unitTest(String cameraMsg)
	{
		...
		// case2.1:initDecode的callback=null;现在都是return 0
		gui.cls_show_msg1(1, "case2.1:setScanningCallback=null测试,不抛出异常即可");
		mSoftEngine.setScanningCallback(null);
		gui.cls_show_msg1(1,"case2.1测试完毕");
		
		// case2.2:未放置二维码或一维码于Camera处，解码超时,SoftEngine方式不支持超时测试项20200702
		...
		gui.cls_show_msg1(1, "case3.2:初始化回调，camera调用预览画面，解码成功后调用解析停止解析数据后可再次解析数据");
		testCameraAny(mIsPlaceCode=true, cameraMsg, scanDefineInfo,1);
		mSoftEngine.stopDecode();
