 ...
 * 					  startRGBDecode(byte[] rgb)、startRGBDecode(String path)、startRGBDecode(InputStream inputStream)
 *                    startRGBDecode(Bitmap bitmap)、stopDecode()、ResultCallBack回调接口
 * related document : 
 ...
 ************************************************************************/
@SuppressLint("NewApi")
public class Scan32 extends UnitFragment
{
	private String fileName=Scan32.class.getSimpleName();
	private final String TESTITEM = "(ScanUtil+Camera2)startYUVDecode";
...
//	private Gui gui;
	...
	public void scan32() 
	{
		...
		if((cameraId=mScanDefineInfo.cameraReal.get(BACK_CAMERA))!=-1)
		{
			...
			Size[] mCameraSizes = getCameraSize(cameraId+"");// 分辨率较多，选部分测试
			strCode = pic[0][2];
			for(int i=0;i<mCameraSizes.length/3;i++)
			{
				gui.cls_show_msg("请放置%s码与%s摄像头，操作完毕任意键继续",pic[0][0],"后置");/**提示具体的分辨率*/
				String resolution = String.format(Locale.CHINA,"%d*%d", mCameraSizes[i*3].getWidth(),mCameraSizes[i*3].getHeight());
				gui.cls_printf(String.format("分辨率设置为%s", resolution).getBytes());
				if((ret=scanUtil.initDecode(mCallback))!=NDK_OK)
				{
					gui.cls_show_msg1_record(fileName, funcName, gKeepTimeErr,"line %d:第%d次，%s初始化失败(%d)", Tools.getLineInfo(),i+1,TESTITEM,ret);
					scanUtil.stopDecode();
					return;
				}

				cameraScan(i*3,cameraId);//扫码
		...
		if((cameraId=mScanDefineInfo.cameraReal.get(USB_CAMERA))!=-1)
		{
			...
			Size[] mCameraSizes = getCameraSize(cameraId+"");// 分辨率较多，选部分测试
			strCode = pic[3][2];
			for(int i=0;i<mCameraSizes.length;i++)
			{
				gui.cls_show_msg("请放置%s码与%s摄像头，操作完毕任意键继续",pic[3][0],"USB");/**提示具体的分辨率*/
				String resolution = String.format(Locale.CHINA,"%d*%d", mCameraSizes[i].getWidth(),mCameraSizes[i].getHeight());
				gui.cls_printf(String.format("分辨率设置为%s", resolution).getBytes());
				if((ret=scanUtil.initDecode(mCallback))!=NDK_OK)
				{
					gui.cls_show_msg1_record(fileName, funcName, gKeepTimeErr,"line %d:第%d次，%s初始化失败(%d)", Tools.getLineInfo(),i+1,TESTITEM,ret);
	...
	/**获取摄像头分辨率*/
	public Size[] getCameraSize(String mCameraId)
	{
		Size[] sizes = null;
...
//	          size = map.getOutputSizes(ImageFormat.YUV_420_888).length;
	          for(Size xx:map.getOutputSizes((ImageFormat.YUV_420_888)))
	          {
	        	  LoggerUtil.d("width="+xx.getWidth()+"==="+xx.getHeight());
	          }
...
//	          LoggerUtil.d("initCamera,mPreviewSize="+mPreviewSize.getWidth()+"==="+mPreviewSize.getHeight());
	 ...
	 * 初始化摄像头摄像头
	 */
	public void initCamera(int index,String mCameraId)
	{
		try {
			 CameraCharacteristics characteristics = cameraManager.getCameraCharacteristics(mCameraId);
			  //支持的STREAM CONFIGURATION
			...
			LoggerUtil.e("angle="+angle);
	          ...
	          //摄像头支持的预览Size数组
	          mPreviewSize = map.getOutputSizes(ImageFormat.YUV_420_888)[index];
...
//		         LoggerUtil.e("setupImageReader="+scanCount);
		        	if(scanUtil!=null&&mIsScanSuc==false)
					{
...
//				          ByteBuffer buffer = image.getPlanes()[0].getBuffer();
				          ...
				          scanUtil.startYUVDecode(data, mPreviewSize.getWidth(),mPreviewSize.getHeight());
				          image.close();
					}
		        }
		    }, mainHandler);
	 ...
	 * YUV调用相机预览解析
	 */
	public void cameraScan(int index,int cameraId){
		timeFlag=false;
		mIsScanSuc = false;

		mSurfaceHolder.setKeepScreenOn(true);
		mSurfaceHolder.addCallback(surfaceCallback);
		initCamera(index,cameraId+"");
