 ...
 * file name 		: BluetoothBase3.java 
 * Author 			: wangxy
 * version 			: 
 * DATE 			: 20180226
 * directory 		: 
 * description 		: 蓝牙底座的RS232串口模块
 * related document : 
 ...
 ************************************************************************/
@SuppressLint("NewApi")
public class BluetoothBase3 extends UnitFragment
{
	
	/*------------global variables definition-----------------------*/
	private final String TESTITEM = "RS232串口模块";
	public final String TAG = BluetoothBase3.class.getSimpleName();
...
//			buf[j] = (byte) (Math.random()*256);
		...
		if (!nlBluetooth.startBluetoothConnA(getBtName(),getBtAddr())){ 
			gui.cls_show_msg1_record(TAG, "BluetoothBase3", gKeepTimeErr,"line %d:连接蓝牙失败（%d）", Tools.getLineInfo(),ret);
				return;
		}
		
		// 测试前置,0代表RS232，8代表usb模拟串口，10代表扫描枪usb主设备，0代表扫描枪COM口
		...
		if((ret=nlBluetooth.portOpen(-1, "115200,8,N,1"))==NDK_OK)
			gui.cls_show_msg1_record(TAG, "BluetoothBase3", gKeepTimeErr,"line %d:参数异常测试失败", Tools.getLineInfo());
		...
		if((ret=nlBluetooth.portOpen(portType, "115200,8,N,1"))!=NDK_OK)
			gui.cls_show_msg1_record(TAG, "BluetoothBase3", gKeepTimeErr,"line %d:串口打开失败", Tools.getLineInfo());
		gui.cls_show_msg1(2, "打开串口后马上做写全0操作");
		if((ret=nlBluetooth.portWrite(portType, MAX_SIZE, buf))!=NDK_OK)
			gui.cls_show_msg1_record(TAG, "BluetoothBase3", gKeepTimeErr,"line %d:写全0失败,ret=%d", Tools.getLineInfo(),ret);
		...
		gui.cls_show_msg1(2, "RS232串口未打开，流程异常测试...");
		nlBluetooth.portClose(portType);
		if((ret=nlBluetooth.portWrite(portType, BUFSIZE_SERIAL, buf))==NDK_OK)
			gui.cls_show_msg1_record(TAG, "BluetoothBase3", gKeepTimeErr,"line %d:写数据失败（%d）", Tools.getLineInfo(),ret);
		
		//portTxSendOver判断指定串口发送缓冲区是否为空,由于NDK接口直接返回0，无实际实现，故该接口不测试
		
		// 取输入缓存,最大4095,所以循环取出
		if ((ret = nlBluetooth.portReadLen(portType, outReceiveLen2)) == NDK_OK)
			gui.cls_show_msg1_record(TAG, "BluetoothBase3", gKeepTimeErr, "line %d:USB串口取缓存区数据失败（%d）", Tools.getLineInfo(), ret);
		
		// 读数据（接收数据）
		Arrays.fill(rbuf, (byte) 0);
		if((ret=nlBluetooth.portRead(portType, BUFSIZE_SERIAL, MAXWAITTIME, outReceiveLen1, rbuf))==NDK_OK)
			gui.cls_show_msg1_record(TAG, "BluetoothBase3", gKeepTimeErr,"line %d:USB串口读数据失败（%d）", Tools.getLineInfo(),ret);
		
		//清USB串口缓存
		if((ret=nlBluetooth.portClrBuf(portType))==NDK_OK)
			gui.cls_show_msg1_record(TAG, "BluetoothBase3", gKeepTimeErr,"line %d:USB串口清缓存失败（%d）", Tools.getLineInfo(),ret);
		
		//测试后置
		gui.cls_show_msg1_record(TAG, "BluetoothBase3", gKeepTimeErr,"%s测试通过", TESTITEM);
	}
	@Override
